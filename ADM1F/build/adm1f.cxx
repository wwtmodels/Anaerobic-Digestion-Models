...
PetscErrorCode IFunctionPassive(TS ts,PetscReal t,Vec U,Vec Udot,Vec F,AppCtx *ctx)
{
  ...
  procT10 = kLa*((x[43])-K_H_co2*p_gas_co2);//
  ...
  f[0] = xdot[0]-1.0/V_liq*(influent[26]*(influent[0]-x[0]))-reac1; 		/* Ssu */
  f[1] = xdot[1]-1.0/V_liq*(influent[26]*(influent[1]-x[1]))-reac2; 		/* Saa */
  f[2] = xdot[2]-1.0/V_liq*(influent[26]*(influent[2]-x[2]))-reac3;		/* Sfa */
  f[3] = xdot[3]-1.0/V_liq*(influent[26]*(influent[3]-x[3]))-reac4;  		/* Sva */
  f[4] = xdot[4]-1.0/V_liq*(influent[26]*(influent[4]-x[4]))-reac5;  		/* Sbu */
  f[5] = xdot[5]-1.0/V_liq*(influent[26]*(influent[5]-x[5]))-reac6;  		/* Spro */
  f[6] = xdot[6]-1.0/V_liq*(influent[26]*(influent[6]-x[6]))-reac7;  		/* Sac */
  f[7] = 1.0/V_liq*(influent[26]*(influent[7]-x[7]))-reac8;			/* Sh2 */
  f[8] = xdot[8]-1.0/V_liq*(influent[26]*(influent[8]-x[8]))-reac9;     	/* Sch4 */
  f[9] = xdot[9]-1.0/V_liq*(influent[26]*(influent[9]-x[9]))-reac10;   	/* SIC */
  f[10] = xdot[10]-1.0/V_liq*(influent[26]*(influent[10]-x[10]))-reac11; 	/* SIN */
  f[11] = xdot[11]-1.0/V_liq*(influent[26]*(influent[11]-x[11]))-reac12;  	/* SI */
  f[12] = xdot[12]-1.0/V_liq*(influent[26]*(influent[12]-x[12]))-reac13; 	/* Xxc */
  f[13] = xdot[13]-1.0/V_liq*(influent[26]*(influent[13]-x[13]))-reac14; 	/* Xch */
  f[14] = xdot[14]-1.0/V_liq*(influent[26]*(influent[14]-x[14]))-reac15; 	/* Xpr */
  f[15] = xdot[15]-1.0/V_liq*(influent[26]*(influent[15]-x[15]))-reac16; 	/* Xli */
  f[16] = xdot[16]-1.0/V_liq*(influent[26]*(influent[16]-x[16]))-reac17; 	/* Xsu */
  f[17] = xdot[17]-1.0/V_liq*(influent[26]*(influent[17]-x[17]))-reac18; 	/* Xaa */
  f[18] = xdot[18]-1.0/V_liq*(influent[26]*(influent[18]-x[18]))-reac19; 	/* Xfa */
  f[19] = xdot[19]-1.0/V_liq*(influent[26]*(influent[19]-x[19]))-reac20; 	/* Xc4 */
  f[20] = xdot[20]-1.0/V_liq*(influent[26]*(influent[20]-x[20]))-reac21; 	/* Xpro */
  f[21] = xdot[21]-1.0/V_liq*(influent[26]*(influent[21]-x[21]))-reac22;	/* Xac */
  f[22] = xdot[22]-1.0/V_liq*(influent[26]*(influent[22]-x[22]))-reac23; 	/* Xh2 */
  f[23] = xdot[23]-1.0/V_liq*(influent[26]*(influent[23]-x[23]))-reac24; 	/* XI */
  f[24] = xdot[24]-1.0/V_liq*(influent[26]*(influent[24]-x[24])); 			/* Scat+ */
  if (ctx->set_Cat_mass) {
    f[24] = f[24]+ctx->Cat_mass; 			/* Scat+ */
  }
  f[25] = xdot[25]-1.0/V_liq*(influent[26]*(influent[25]-x[25])); 			/* San- */
...
PetscErrorCode IFunctionActive1(TS ts,PetscReal t,Vec U,Vec Udot,Vec F,AppCtx *ctx)
{
  ...
  /* Process Rates*/  
  proc1 = k_dis*x_a[12];
  proc2 = k_hyd_ch*x_a[13];
  proc3 = k_hyd_pr*x_a[14];
  proc4 = k_hyd_li*x_a[15];
  proc5 = k_m_su*x_a[0]/(K_S_su+x_a[0])*x_a[16]*inhib[0];
  proc6 = k_m_aa*x_a[1]/(K_S_aa+x_a[1])*x_a[17]*inhib[0];
  proc7 = k_m_fa*x_a[2]/(K_S_fa+x_a[2])*x_a[18]*inhib[1];
  proc8 = k_m_c4*x_a[3]/(K_S_c4+x_a[3])*x_a[19]*x_a[3]/(x_a[3]+x_a[4]+eps)*inhib[2];
  proc9 = k_m_c4*x_a[4]/(K_S_c4+x_a[4])*x_a[19]*x_a[4]/(x_a[3]+x_a[4]+eps)*inhib[2];
  proc10 = k_m_pro*x_a[5]/(K_S_pro+x_a[5])*x_a[20]*inhib[3];
  proc11 = k_m_ac*x_a[6]/(K_S_ac+x_a[6])*x_a[21]*inhib[4];
  proc12 = k_m_h2*x_a[7]/(K_S_h2+x_a[7])*x_a[22]*inhib[5];//
  ...
  procT8 = kLa*(x_a[7]-16.0*K_H_h2*p_gas_h2); //
  procT9 = kLa*(x_a[8]-64.0*K_H_ch4*p_gas_ch4); //
  procT10 = kLa*((x_a[43])-K_H_co2*p_gas_co2);//
  ...
  f_a[25] = xdot[25]-1.0/V_liq*(influent[26]*(influent[25]-x_a[25])); 		  	/* San- */
...
int main(int argc,char **argv)
{
  ...
  PetscBool      steady, set_Vliq, set_Vgas;
  AppCtx         ctx;
  // PetscScalar    *u;
  AdolcCtx       *adctx;
